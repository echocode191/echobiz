// /workspaces/echobiz/src/components/CourseIdeas.jsx
...
const CourseIdeas = () => {
  ...
  const getBookStyle = (course, isHovered) => ({
  ...
  const openFullView = (course) => {
    setFullViewItem(course);
  };
    ...
    <section style={sectionStyle}>
      ...
      <div style={gridStyle}>
        {courseIdeas.map(course => (
          <div 
            key={course.id}
            style={getBookStyle(course, hoveredId === course.id)}
            onMouseEnter={() => setHoveredId(course.id)}
            onMouseLeave={() => setHoveredId(null)}
          >
            <div style={bookSpineStyle(course.color)}></div>
            <img 
            ...
            <p style={descriptionStyle} title={course.description}>{course.description}</p>
            <p style={contentStyle} title={course.content}>{course.content}</p>
            
            {expandedId === course.id && !course.isPaid && (
            ...
            <div style={metaStyle}>
              <span style={categoryStyle} title={course.category}>{course.category}</span>
            ...
            {course.isPaid ? (
              ...
              <>
                ...
                >
                  {expandedId === course.id ? 'Hide' : 'Free'}
                </button>
                <button 
                  style={viewNowButtonStyle}
                  onClick={() => openFullView(course)}
                >
                  View Now
                </button>
              </>
        ...
        <div style={fullViewOverlayStyle} onClick={closeFullView}>
          <div style={fullViewContentStyle} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ backgroundColor: '#f9fafb', padding: '1rem', borderRadius: '0.5rem', borderLeft: '4px solid #4F46E5' }}>
              <h3 style={{ fontSize: '1.25rem', marginBottom: '1rem', color: '#1F2937' }}>Course Details</h3>
